...
function makeRepoRow(repo, userLogin, extPath) {
  ...
  const iconName = getRepoIcon(repo, userLogin);
 ...
 * @param {PopupMenu.PopupMenu} menu target menu
 */
function addEmptyState(menu) {
  const row = new PopupMenu.PopupMenuItem(_("No recent repositories"), {
    reactive: false,
    can_focus: false,
  });
...
function addNoTokenState(menu) {
  const row = new PopupMenu.PopupMenuItem(_("Create GitHub Personal Access Token"), {
    reactive: true,
    can_focus: true,
  });
...
function addLoadingState(menu) {
  const row = new PopupMenu.PopupMenuItem(_("Loading..."), {
    reactive: false,
    can_focus: false,
  });
...
function addErrorState(menu, message) {
  const row = new PopupMenu.PopupMenuItem(_(message), {
    reactive: false,
    can_focus: false,
  });
...
function addViewProfile(menu, user) {
  ...
  row.connect("activate", () => {
    if (user?.html_url) {
      Gio.AppInfo.launch_default_for_uri(user.html_url, null);
    }
  });
...
export function buildMenu(menu, state, extPath, actions = {}) {
  ...
  if (error) {
    addErrorState(menu, error);
  } else if (!hasToken) {
    addNoTokenState(menu);
  } else if (!user) {
    addLoadingState(menu);
