...
function getSession() {
  if (!_session) {
    ...
    _session.user_agent = "github-tools/1.0";
  }
  return _session;
}
...
async function request(url, token) {
  ...
  for (const [key, value] of Object.entries(makeHeaders(token)))
    message.request_headers.append(key, value);

  const bytes = await sess.send_and_read_async(
    message,
    GLib.PRIORITY_DEFAULT,
    null,
  );

  if (message.status_code === 401) throw new InvalidTokenError();
...
export async function fetchUser(token) {
  const data = await request(`${API_BASE}/user`, token);
  return {
    login: data.login,
